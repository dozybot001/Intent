...
两种对象：**intent**（目标）和 **checkpoint**（步骤及理由）。全部 JSON。任何 agent 平台都可以读取。
...
**有 `.intent/`** — 新 session 开始。Agent 运行 `itt inspect`，看到 active intent（"将认证迁移到 JWT"）、最后一个 checkpoint（"添加 refresh token——未完成"）、以及理由（"token rotation 未做，安全优先级"）。它说：*"我来实现 token rotation。"*
