...
恢复一个暂停的 intent。如果只有一个 suspended intent，不需要指定 ID。
...
没有 active intent 时，`intent`、`latest_snap` 为 `null`，`candidate_snaps` 和 `suspended_intents` 为 `[]`，`suggested_next_action` 推荐 `itt start`（若有 suspended intent 则推荐 `itt resume`）。
