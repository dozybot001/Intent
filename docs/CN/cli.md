...
`.intent/` 是本地工作区元数据，不是仓库源码的一部分；它应保持在 Git 历史之外，并由 `.gitignore` 忽略。
...
这些命令用于把本地 `.intent/` 同步到 IntHub 首版协作层。

#### login

写入本地 IntHub 运行时配置，至少包含 API base URL。
...
- 写入 `.intent/hub.json`
- `token` 只保存在本地运行时配置中，不属于语义对象

#### link

将当前 GitHub 仓库与一个 IntHub project 建立绑定，并初始化本地 workspace 绑定信息。
...
- 读取当前 `origin` remote，并要求其为 GitHub 仓库
- 初始化或复用本地 `workspace_id`
- 将返回的 `project_id`、`workspace_id` 与 `repo_binding` 写入 `.intent/hub.json`

#### sync

把当前 `.intent/` 快照与 Git 上下文打包并推送到 IntHub。
...
- 上传完整对象快照，而不是增量 patch
- 同步包含 `branch`、`head_commit`、`dirty`、`remote_url`
- `--dry-run` 仅输出将要发送的 payload，不进行网络请求
...
| `HUB_NOT_CONFIGURED` | 未配置 IntHub API base URL |
| `NOT_LINKED` | 当前工作区尚未完成 IntHub 绑定 |
| `PROVIDER_UNSUPPORTED` | 当前 Git remote 不是受支持的 provider |
| `NETWORK_ERROR` | 无法连接 IntHub 服务端 |
| `SERVER_ERROR` | IntHub 返回错误状态或非法 JSON |
