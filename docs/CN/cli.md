...
| 命令 | 作用 |
| --- | --- |
| `itt intent create WHAT --query Q [--why W] [--origin LABEL]` | 创建一个新的 intent |
...
| 命令 | 作用 |
| --- | --- |
| `itt snap create WHAT [--intent ID] [--query Q] [--why W] [--next N] [--origin LABEL]` | 创建语义快照（`what` = 做了什么；`why` = 为什么；`next` = 下一步） |

### Decision

| 命令 | 作用 |
| --- | --- |
| `itt decision create WHAT --why W [--query Q] [--origin LABEL]` | 创建一个新的 decision |
...
语义快照。`what` = 做了什么，`why` = 为什么，`next` = 下一步。
