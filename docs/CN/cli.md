...
| `itt snap create WHAT [--intent ID] --why W [--query Q] [--next N]` | 创建语义快照 | `--intent` 唯一 active 时自动推断。`origin` 自动填充。 |
| `itt decision create WHAT --why W [--query Q]` | 创建长期约束 | 自动挂载所有 active intent。`origin` 自动填充。 |
