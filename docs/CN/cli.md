...
| 命令 | 作用 |
| --- | --- |
| `itt intent create TITLE --query Q [--why R] [--origin LABEL]` | 创建一个新的 intent |
...
| 命令 | 作用 |
| --- | --- |
| `itt snap create TITLE [--intent ID] [--query Q] [--why R] [--next N] [--origin LABEL]` | 创建语义快照（`title` = 做了什么；`why` = 为什么；`next` = 下一步） |

### Decision

| 命令 | 作用 |
| --- | --- |
| `itt decision create TITLE --why R [--query Q] [--origin LABEL]` | 创建一个新的 decision |
...
语义快照。`title` = 做了什么，`why` = 为什么，`next` = 下一步。

```bash
itt snap create "超时改为30s并改为异步刷新" \
  --query "为什么登录超时5秒" \
  --why "refresh flow 中的竞态阻塞了登录流程。改为异步刷新解耦路径。" \
  --next "Token refresh 端点仍硬编码 — 不同服务，需要单独处理。"
```

说明：

- `--why` 必填；`--intent` 在恰好一个 `active` intent 时自动推断
