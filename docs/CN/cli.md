...
| 命令 | 作用 |
| --- | --- |
| `itt snap create TITLE [--intent ID] [--origin LABEL] --summary S` | 创建语义快照（仅有一个 `active` intent 时可省略 `--intent`） |
...
`create` 将 AI 的推理持久化为语义快照。Git 记录代码怎么变了；snap 记录为什么这么改。

```bash
itt snap create "Raise timeout to 30s" \
  --summary "竞态在 refresh flow 而不是 login handler。超时改到 30s。Token refresh 还在硬编码 — 不同服务，需要单独处理。"

itt snap create "Raise timeout to 30s" \
  --intent intent-001 \
  --summary "竞态在 refresh flow 而不是 login handler。"
```
