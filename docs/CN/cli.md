...
| 命令 | 作用 |
| --- | --- |
| `itt snap create TITLE [--intent ID] [--origin LABEL] --summary S` | 创建语义检查点（仅有一个 `active` intent 时可省略 `--intent`） |
...
| `source_query` | ✓ | | ✓ | 触发该对象的原始用户话语 |
| `rationale` | ✓ | | ✓ | 为什么这个目标/约束重要 |
| `origin` | ✓ | ✓ | ✓ | 从环境自动检测（如 `claude-code`、`cursor`、`codex-desktop`） |
| `summary` | | ✓ | | 做了什么、还剩什么、什么上下文影响下一步 |
| `feedback` | | ✓ | | 用户反馈，通过 `snap feedback` 覆盖写入 |
| `intent_id` | | ✓ | | 所属 intent |
...
所有字段**创建后不可变**，唯一例外是 `feedback`（通过 `snap feedback` 覆盖写入）。
...
`create` 在 active intent 下写入一个语义检查点。`feedback` 单独设置，覆盖语义。
...
- 纠正旧 checkpoint 应追加 feedback 或写新 snap，不是回滚
