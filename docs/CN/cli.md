...
| `itt intent create WHAT [--why W]` | 创建 intent | 自动挂载所有 active decision。`origin` 自动填充。 |
...
| `itt snap create WHAT [--why W]` | 创建语义快照 | 自动挂载到 active intent。多个时 CLI 返回候选列表，需指定 `--intent ID`。 |
| `itt decision create WHAT [--why W]` | 创建长期约束 | 自动挂载所有 active intent。`origin` 自动填充。 |
