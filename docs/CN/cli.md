...
| 命令 | 作用 |
| --- | --- |
| `itt snap create TITLE [--intent ID] [--query Q] [--rationale R] [--next N] [--origin LABEL]` | 创建语义快照（`title` = 做了什么；`rationale` = 为什么；`next` = 下一步） |
...
| `rationale` | ✓ | ✓ | ✓ | Intent: 为什么要做。Snap: 为什么这么做。Decision: 为什么有这个约束。 |
...
语义快照。`title` = 做了什么，`rationale` = 为什么，`next` = 下一步。

```bash
itt snap create "超时改为30s并改为异步刷新" \
  --query "为什么登录超时5秒" \
  --rationale "refresh flow 中的竞态阻塞了登录流程。改为异步刷新解耦路径。" \
  --next "Token refresh 端点仍硬编码 — 不同服务，需要单独处理。"
```

说明：

- `--rationale` 必填；`--intent` 在恰好一个 `active` intent 时自动推断
