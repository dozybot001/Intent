...
### 另一种方式：Session 粒度（回溯模式）

不必按 query 粒度实时创建 intent 和 snap，也可以按 **session 粒度**工作：一个 session 对应一个 intent，snap 对应工作中的里程碑。在 session 结束后回顾整理，此时全貌清晰，判断更精准，信噪比更高。两种模式都合法——根据场景选择。
