...
class IntentRepository:
    ...
    def setup(self, platforms: Optional[List[str]] = None) -> Dict[str, Any]:
        self.ensure_git()
        ...
        from .snippets import AGENTS_MARKER, AGENTS_MD, CLAUDE_MD, CURSOR_MDC, MARKER

        targets = platforms or self._detect_platforms()
        created: List[str] = []
        updated: List[str] = []
        skipped: List[str] = []
        ...
        for platform in targets:
            if platform == "claude":
                result = self._setup_file(
                    self.cwd / "CLAUDE.md", CLAUDE_MD, MARKER,
                )
            elif platform == "cursor":
                rules_dir = self.cwd / ".cursor" / "rules"
                rules_dir.mkdir(parents=True, exist_ok=True)
                result = self._setup_file(
                    rules_dir / "intent.mdc", CURSOR_MDC, "alwaysApply",
                )
            elif platform == "agents":
                result = self._setup_file(
                    self.cwd / "AGENTS.md", AGENTS_MD, AGENTS_MARKER,
                )
            else:
                continue
    ...
    def _detect_platforms(self) -> List[str]:
        platforms: List[str] = []
        if (self.cwd / "CLAUDE.md").exists():
            platforms.append("claude")
        if (self.cwd / ".cursor").exists():
            platforms.append("cursor")
        if (self.cwd / "AGENTS.md").exists():
    ...
    def _setup_file(self, path: Path, snippet: str, marker: str) -> str:
        if path.exists():
            ...
            # append
            separator = "\n" if content.endswith("\n") else "\n\n"
            path.write_text(content + separator + snippet, encoding="utf-8")
            return "updated"
        else:
